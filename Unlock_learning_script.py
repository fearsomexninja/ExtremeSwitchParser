...
filename = "/usr/local/cfg/{0}-{1}-{2}_WAP_unlock.txt".format(date.year, date.month,date.day)
f = open(filename,'w')
...
#	if inline_entry:
	...
	while i<len(OUIList):
		...
		if ouicheck:
			...
			lock_learning_command = "conf port "+item[1]+" vlan "+info_detail_match.group(1)+" unlock-learning"
...
i=0
f.write("these {0} ports were unlocked:\n".format(locked_ports))
